...
class Recipe:
    ...
    def __init__(self, buildout, name, options):
        ...
        if ("input" not in options and
                "inline" not in options and "url" not in options):
            self.logger.error(
        ...
        if (options.get('input') and options.get('inline') or
                options.get('input') and options.get('url')):
        ...
        if "inline" in options:
            ...
            if self.url is None:
                msg = "Input file '{}' does not exist.".format(self.input)
            else:
                msg = self.msg
